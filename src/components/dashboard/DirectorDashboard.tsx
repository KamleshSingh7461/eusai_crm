    ...
    return (
        <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-[#172B4D] mb-1 md:mb-2 tracking-tight">Director Master Hub</h1>
                    <p className="text-[#6B778C] text-sm md:text-lg">Consolidated view of team performance and project health.</p>
                </div>
            ...
            {activeSection === 'overview' && (
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        ...
                        <MetricCard icon={Target} label="Market Target" value={`${stats.marketCoverage}%`} color="purple" />
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="space-y-8">
                            ...
                            {/* Target achievement mockup */}
                            <div className="bg-white p-6 border border-[#DFE1E6] rounded-sm shadow-sm">
                                <h3 className="text-xs font-bold text-[#6B778C] uppercase tracking-widest mb-4">Target Achievement</h3>
            ...
            {activeSection === 'team' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {employees.map((emp: any) => (
                        <div key={emp.id} className="bg-white border border-[#DFE1E6] p-5 rounded-sm hover:shadow-md transition-all group">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="w-12 h-12 rounded-full bg-[#F4F5F7] flex items-center justify-center text-[#172B4D] font-bold text-lg overflow-hidden border border-[#DFE1E6]">
                                    {emp.image ? <img src={emp.image} alt={emp.name} className="w-full h-full object-cover" /> : emp.name.charAt(0)}
                                </div>
                                <div>
                                    <h3 className="font-bold text-[#172B4D] group-hover:text-[#0052CC] transition-colors">{emp.name}</h3>
                                    <p className="text-xs text-[#6B778C] font-medium">{emp.role}</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[#F4F5F7]">
                                <div className="text-center">
                                    <div className="text-lg font-bold text-[#172B4D]">{emp.pendingTasks}</div>
                                    <div className="text-[10px] text-[#6B778C] font-bold uppercase tracking-wider">Open Tasks</div>
                                </div>
                                <div className="text-center border-l border-[#F4F5F7]">
                                    <div className="text-lg font-bold text-[#0052CC]">{emp.pendingMilestones}</div>
                                    <div className="text-[10px] text-[#6B778C] font-bold uppercase tracking-wider">Milestones</div>
                                </div>
                            </div>
                            <Button
                                variant="ghost"
            ...
            {activeSection === 'projects' && (
                <div className="bg-white border border-[#DFE1E6] rounded-sm shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-[#FAFBFC] border-b border-[#DFE1E6]">
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider">Project Name</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider">Manager</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider">Status</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider text-center">Tasks</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider text-center">Milestones</th>
                                <th className="px-6 py-4 text-[11px] font-bold text-[#6B778C] uppercase tracking-wider w-48">Progress</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-[#DFE1E6]">
                            {projects.map((proj: any) => (
                                <tr key={proj.id} className="hover:bg-[#F4F5F7] transition-colors cursor-pointer group">
                                    <td className="px-6 py-4">
                                        <div className="font-bold text-[#172B4D] group-hover:text-[#0052CC]">{proj.name}</div>
                                    </td>
                                    <td className="px-6 py-4 text-sm text-[#42526E]">{proj.managerName}</td>
                                    <td className="px-6 py-4">
                                        <span className={cn(
                                    ...
                                    <td className="px-6 py-4 text-center font-medium text-[#172B4D]">{proj.taskCount}</td>
    ...
    return (
        <div className="group bg-white p-6 rounded-sm border border-[#DFE1E6] shadow-sm hover:shadow-md transition-all flex items-center gap-5 relative overflow-hidden">
            ...
            <div className="relative z-10">
                ...
                <div className="text-[10px] text-[#6B778C] font-bold uppercase tracking-widest mt-0.5">{label}</div>
            </div>
        </div>
    );
}
...
function SectionHeader({ title, badge }: { title: string, badge: string }) {
    return (
        <div className="flex items-center justify-between mb-4 px-1">
            <h2 className="text-[11px] font-bold text-[#6B778C] uppercase tracking-[0.2em]">{title}</h2>
    ...
    return (
        <div className="space-y-1.5">
            <div className="flex justify-between text-[11px] font-medium">
                ...
                <span className="text-[#172B4D] font-bold">{displayCurrent} / {displayTarget}</span>
            </div>
