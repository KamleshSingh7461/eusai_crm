    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-[#172B4D]">Team Leader Dashboard</h1>
                    <p className="text-[#6B778C]">Manage your team's performance and your own pipeline.</p>
                </div>
            ...
            <section className="bg-white border border-[#DFE1E6] rounded-sm p-6 shadow-sm">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <h2 className="text-lg font-bold text-[#172B4D]">Team Pulse</h2>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="p-4 bg-blue-50/50 rounded-sm border border-blue-100">
                        <div className="text-xs font-bold text-[#6B778C] uppercase mb-1">Total Team Revenue</div>
                    ...
                    <div className="p-4 bg-purple-50/50 rounded-sm border border-purple-100">
                        <div className="text-xs font-bold text-[#6B778C] uppercase mb-1">Agreements Closed</div>
                        <div className="text-2xl font-bold text-purple-700">{stats.agreements}</div>
                        <div className="text-xs text-[#6B778C] mt-1">Across {team.length} members</div>
                    </div>
                    <div className="p-4 bg-orange-50/50 rounded-sm border border-orange-100">
                        <div className="text-xs font-bold text-[#6B778C] uppercase mb-1">Actions Due</div>
                ...
                <div className="space-y-3">
                    <h3 className="text-xs font-bold text-[#6B778C] uppercase tracking-wider mb-3">Direct Reports</h3>
                    {team.length === 0 ? (
                        <div className="text-sm text-[#6B778C] py-4 bg-[#FAFBFC] text-center rounded-sm">
                        ...
                        team.map((member, i) => (
                            <div key={i} className="flex items-center justify-between p-3 hover:bg-[#FAFBFC] rounded-sm border border-transparent hover:border-[#DFE1E6] transition-all">
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-[#EBECF0] flex items-center justify-center text-xs font-bold text-[#42526E]">
                                        {member.name.charAt(0)}
                                    </div>
                                    <div>
                                        <div className="text-sm font-medium text-[#172B4D]">{member.name}</div>
                                        <div className="text-xs text-[#6B778C]">{member.activity}</div>
                                    </div>
                                </div>
            ...
            <section>
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-bold text-[#172B4D]">Pending Team Approvals</h2>
                    </div>
                ...
                {pendingApprovals.length === 0 ? (
                    <div className="p-8 bg-white border border-dashed border-[#DFE1E6] rounded-sm text-center">
                        ...
                        <h3 className="font-bold text-[#42526E]">All Clear!</h3>
                        <p className="text-sm text-[#6B778C]">No pending expenses or requests from your team.</p>
                    </div>
                ) : (
                        ...
                        {pendingApprovals.map((approval) => (
                            <div key={approval.id} className="bg-white border border-[#DFE1E6] rounded-sm p-4 hover:shadow-md transition-shadow">
                                <div className="flex items-center justify-between mb-3">
                                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#6B778C] bg-[#F4F5F7] px-2 py-0.5 roundedElement">
                                        {approval.type}
                                    </span>
                                    <span className="text-sm font-bold text-[#172B4D]">{approval.amount}</span>
                                </div>
                                <h4 className="font-bold text-[#172B4D] mb-1 line-clamp-1">{approval.title}</h4>
                                <p className="text-xs text-[#6B778C] mb-4 flex items-center gap-1">
                                    Submitted by <span className="font-medium text-[#42526E]">{approval.submittedBy}</span>
                                </p>
                                <div className="flex gap-2 pt-3 border-t border-[#F4F5F7]">
                                    ...
                                    <button
                                        ...
                                        className="flex-1 py-1.5 bg-white border border-[#DFE1E6] text-[#42526E] text-[11px] font-bold rounded-sm hover:bg-[#F4F5F7] transition-colors"
                                    >
                                        Reject
                                    </button>
                                </div>
            ...
            <section>
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <h2 className="text-lg font-bold text-[#172B4D]">My Personal Pipeline</h2>
                </div>
